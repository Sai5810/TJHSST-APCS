...
class AssemblyLine {
    ...
    public AssemblyLine(int nDisks, int maxRadius) {
        ...
        for(int i = 0; i < nDisks; i++) {
            ...
            assemblyLineIn.add(new Disk(ran.nextInt(maxRadius) + 1));
        }
        assemblyLineOut = new LinkedList<>();
        robotArm = new Pyramid(new Stack<>());
    }
    ...
    //Part C
    public void process() {
        Disk prev = assemblyLineIn.remove();
        robotArm.add(prev);
        while(!assemblyLineIn.isEmpty()) {
            Disk c = assemblyLineIn.remove();
            if(c.compareTo(prev) > 0) {
                robotArm.add(c);
            } else {
                Pyramid copy = new Pyramid(new Stack<>());
                while (!robotArm.isEmpty()) {
                    copy.push(robotArm.pop());
                }
                assemblyLineOut.add(copy);
                robotArm = new Pyramid(new Stack<>());
                robotArm.push(c);
            }
            prev = c;
...
class Pyramid extends Stack < Disk > {
    private final Stack<Disk> val;
    public Pyramid(Stack<Disk> s) {
        val = s;
    }
